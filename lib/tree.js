...
class TreeNode {
  constructor(keys, children, count) {
  ...
  setValue(context, i, value) {
    ...
    // NOTE: resetting 'count' might not be necessary here but resetting
    // state for other potential uses of map/reduce will be, and this is a
    // test run for that.
    delete this.count
  }
  ...
  split(context) {
    ...
    const right = context.createTreeNode(0, 0, 0, true, right_node)
