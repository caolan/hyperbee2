...
module.exports = class WriteBatch {
  ...
  async flush(reducers) {
    ...
    try {
      ...
      this.length = length
      this.root = context.createTreeNode(0, seq, 0, changed, value, null, null)
  ...
  // Returns: [updated node count, reducer result]
  async _materializeReducer(ptr, name, reduce) {
    // Already calculated?
    const existing = ptr.reducers?.[name]
    if (existing !== null && existing !== undefined) return [0, existing]
    ...
    for (let i = 0, len = v.children.length; i < len; i++) {
      ...
      const [count, result] = await this._materializeReducer(c, name, reduce)
      updateCount += count
      rereduce.push(result)
    }
    ...
    // Note: a reducer that returns undefined or null as its result would break
    // detection of whether the reducer has been calculated for the current
    // node. However, reducers MUST return JSON representable values so they
    // can be encoded in the block anyway so undefined is already unsupported.
  ...
  async _put(key, value) {
    ...
    while (status === NEEDS_SPLIT) {
      ...
      if (parent) {
        ...
        this.root = this.tree.context.createTreeNode(
          0,
          0,
          0,
          true,
  ...
  async _flush(reducers) {
    // TODO: this.root is always set in flush() so why check if falsy here?
    ...
    if (reducers) {
      for (const [name, reducer] of Object.entries(reducers)) {
        const [updateCount, _result] = await this._materializeReducer(this.root, name, reducer)
        // Only consider applied if a subtree changed. If the only change
        // being flushed is a recalculation of reducer at root it's probably
        // not worth writing a batch for (and we've just recreated root so
