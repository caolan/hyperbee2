...
const Block2 = getEncoding('@bee/block-2')

// type in this context = data schema version
const TYPE_COMPAT = 10 // hyperbee1 format, all blocks start with 0x10
// 0 = hyperbee2 format 0
// 1 = hyperbee2 format 1
// 2 = hyperbee2 format 2: encode pointer seq values relative to block seq

exports.TYPE_COMPAT = TYPE_COMPAT
exports.TYPE_LATEST = 2
...
function encodeBlock(block, seq) {
  if (block.type === 2) {
    block = makePointersRelative(block, seq)
    return c.encode(Block2, block)
  }
...
function decodeBlock(buffer, seq) {
  ...
  if (type === 2) {
    let block = Block2.decode(state)
    block = makePointersAbsolute(block, seq)
    return block
  }
...
function makePointersRelative(block, seq) {
  // Assuming block is still in use by lib/write.js and is not
  // safe to mutate.
  if (block.tree) {
    block = {
      ...block,
      tree: block.tree.map((tree) => {
        return {
          ...tree,
          keys: tree.keys.map((op) => {
            op = { ...op }
            if (op.pointer) {
              op.pointer = { ...op.pointer, seq: op.pointer.seq - seq }
            }
            return op
          })
        }
...
function makePointersAbsolute(block, seq) {
  // Assuming block is owned by encoding module at this point and
  // therefore is safe to mutate.
  if (block.tree) {
    for (const tree of block.tree) {
      for (const op of tree.keys) {
        if (op.pointer) {
          op.pointer.seq += seq
        }
      }
