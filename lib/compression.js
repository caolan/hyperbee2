...
class CompressedArray {
  constructor(delta) {
    ...
    // Populate entries
    for (const d of delta) this._apply(d)
  }

  // Apply a DeltaOp / DeltaCohort to entries
  _apply(d) {
    if (d.type === OP_INSERT) {
      return insert(this.entries, d.index, d.pointer)
    }
    if (d.type === OP_DEL) {
      return del(this.entries, d.index)
    }
    if (d.type === OP_SET) {
      return set(this.entries, d.index, d.pointer)
    }
    if (d.type === OP_COHORT) {
      for (const dd of d.deltas) this._apply(dd)
      return
    }
    return false
  }
  ...
  // Take DeltaOps from end of deltas Array, leaving only DeltaCohorts.
  _takeDeltaOps() {
    const direct = []
  ...
  // Creates a new DeltaCohort containing an OP_INSERT for each entry in
  // CompressedArray.
  _flushToCohort() {
  ...
  flush(max, min) {
    ...
    // NOTE: because cohorts are included in the deltas Array, the
    // effective max DeltaOps that can be added before they are
    // grouped into a DeltaCohort shrinks by 1 each time a cohort is added.
    if (this.delta.length <= max && !overflow) return this.delta

    const direct = this._takeDeltaOps()

    if (direct.length > min && direct.length < this.entries.length && !overflow) {
      // Group trailing DeltaOps into a new DeltaCohort
      this.delta.push(new DeltaCohort(true, null, direct))
    } else {
      // Drop existing deltas and replace with a new DeltaCohort.
      this.delta = [this._flushToCohort()]
    }
    return this.delta
  }
}
